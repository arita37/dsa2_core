...
In this post I'll talk about approaching natural language processing problems on Kaggle.
...
import os
print( os.getcwd() )
dir0 = "data/spooky-author/"
print( os.system("ls " + dir0) )


# In[8]:


train = pd.read_csv( dir0 + "/train.zip" )
test = pd.read_csv(  dir0 + "/test.zip" )
sample = pd.read_csv( dir0 + "/sample_submission.zip" )
...
filename = wget.download("http://www-nlp.stanford.edu/data/glove.840B.300d.zip")
...
model.fit(xtrain_tfv, ytrain, batch_size=64, 
          epochs=2, verbose=1, 
...
xtrain_seq = token.texts_tfo_sequences(xtrain)
